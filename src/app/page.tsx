...
function MealRecommender() {
  ...
  const getRecommendations = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      // AI note
      const notes = [
        people >= 4 ? `For ${people} people, our Squad Pack saves you ₹743!` : "",
        spice === "hot" ? "Dragon Chicken Biryani is our spiciest — you'll love it!" : "",
        ...
        total <= budget ? `Great news — this fits your ₹${budget} budget perfectly!` : `Tip: Try a combo to save more within your budget.`,
      ].filter(Boolean);
  ...
  const steps = [
    ...
    { q: "Food preference?", options: [{ label: "🟢 Veg Only", val: "veg" }, { label: "🔴 Non-Veg", val: "nv" }, { label: "🟡 Both", val: "any" }] },
    { q: "Spice level?", options: [{ label: "😌 Mild", val: "mild" }, { label: "😋 Medium", val: "medium" }, { label: "🔥 Extra Hot", val: "hot" }] },
    { q: "Budget?", options: [{ label: "₹200", val: 200 }, { label: "₹500", val: 500 }, { label: "₹1000", val: 1000 }, { label: "₹2000+", val: 2000 }] },
  ];
        ...
        {!recommendations ? (
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-dark border border-white/10 rounded-2xl p-8 text-center">
              ...
              {step > 0 && <button onClick={() => setStep(step - 1)} className="mt-4 text-xs text-white/30 hover:text-white/60 transition-colors">← Back</button>}
            </div>
          </div>
        ) : isAnimating ? (
          ...
          <div className="space-y-6 animate-fade-in">
                ...
                <div key={item.id} className="bg-dark border border-white/10 rounded-xl p-4 flex gap-4 hover:border-gold/30 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] ${item.type === "veg" ? "text-green-400" : "text-red-400"}`}>{item.type === "veg" ? "🟢" : "🔴"}</span>
                    </div>
            ...
            {recommendations.combo && (
              <div className="bg-gradient-to-r from-gold/10 to-brand/10 border border-gold/20 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-gold font-display text-lg">💡 Better Deal</span>
                  <span className="text-[10px] bg-gold/20 text-gold px-2 py-0.5 rounded-full font-bold">{recommendations.combo.tag}</span>
                </div>
                <div className="text-white font-semibold">{recommendations.combo.name} — <span className="text-gold">₹{recommendations.combo.price}</span> <span className="text-white/30 text-sm line-through">₹{recommendations.combo.was}</span></div>
                <p className="text-white/50 text-sm mt-1">{recommendations.combo.items}</p>
                <div className="text-whatsapp text-sm font-bold mt-2">You save ₹{recommendations.combo.save}!</div>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-dark border border-white/10 rounded-xl p-5">
              <div>
                ...
                <div className="font-display text-3xl text-white">₹{recommendations.total}</div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => { setRecommendations(null); setStep(0); }} className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-full text-sm text-white/60 hover:text-white hover:bg-white/10 transition-colors">🔄 Start Over</button>
                <a href={`${WHATSAPP_URL}?text=${encodeURIComponent(`Hi! AI recommended: ${recommendations.items.map(i => i.name).join(", ")}. Total: ₹${recommendations.total}`)}`} target="_blank" rel="noopener noreferrer" className="btn-wa !py-2.5 text-sm">💬 Order This</a>
              </div>
            </div>
          </div>
        )}
...
function ReviewsSection() {
  ...
  const aiAnalysis = {
    ...
    keyInsights: [
      { label: "Food Quality", score: 98, icon: "🍛", detail: "Consistently praised for authentic taste" },
      { label: "Portion Size", score: 95, icon: "📏", detail: "Generous portions mentioned frequently" },
      { label: "Delivery Speed", score: 92, icon: "🛵", detail: "Fast delivery, food arrives hot" },
      { label: "Value for Money", score: 96, icon: "💰", detail: "Great prices, combo deals loved" },
    ],
    topMentions: ["Chicken Dum Biryani", "Dragon Chicken", "WhatsApp ordering", "Consistent quality", "Office orders"],
    aiSummary: "Customers consistently highlight the authentic dum biryani preparation and generous portions. The WhatsApp ordering system is a standout feature, especially for office bulk orders. Dragon Chicken Biryani has emerged as a fan favourite for spice lovers. Repeat orders are very common — a strong indicator of customer satisfaction.",
  };
    ...
    <section id="reviews" className="section-pad bg-dark">
      <div ref={ref} className="reveal max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          >
            <span className="text-purple-400 text-sm font-semibold group-hover:text-purple-300">🤖 {showAI ? "Hide" : "Show"} AI Analysis</span>
          </button>
        </div>
            ...
            <div className="bg-gradient-to-r from-purple-500/10 via-brand/5 to-purple-500/10 border border-purple-500/20 rounded-2xl p-6 md:p-8">
              <div className="flex flex-wrap items-center gap-6 mb-6">
                <div className="text-center">
                  ...
                  <div className="flex gap-0.5 justify-center mt-1">{[...Array(5)].map((_, i) => <span key={i} className="text-gold text-sm">★</span>)}</div>
              ...
              <div className="mt-6 pt-4 border-t border-white/5">
                <div className="text-xs text-white/30 uppercase tracking-wider mb-3">🔥 Most Mentioned</div>
...
function ChatBot() {
  ...
  const [messages, setMessages] = useState<{ from: string; text: string }[]>([
    { from: "bot", text: "Hey! 🐋 I'm Pothuraju AI — your personal biryani assistant. Ask me anything!" },
    { from: "bot", text: "Try: \"What's your best biryani?\", \"Veg options under ₹200\", \"Book a table\", or just tell me what you're craving! 🍛" },
  ]);
  ...
  const getAIResponse = (msg: string): string | null => {
    ...
    // Booking flow
    if (bookingStep > 0) {
      if (bookingStep === 1) { setBooking(b => ({ ...b, name: msg })); setBookingStep(2); return `Nice to meet you, ${msg}! 😊 How many guests?`; }
    ...
    // Greetings
    if (/^(hi|hello|hey|sup|yo|namaste|hii+)\b/.test(lower)) return "Hey there! 🐋 Welcome to Pothuraju Biryani! What are you craving today? I can help with menu recommendations, prices, combos, or booking a table!";

    // Book a table
    if (lower.includes("book") || lower.includes("table") || lower.includes("reserv")) { setBookingStep(1); return "Let's book a table! 🍽️ What's your name?"; }
    ...
    if (lower.includes("order") && (lower.includes("whatsapp") || lower.includes("place"))) {
      ...
      return "Opening WhatsApp for your order! 🛵 Our team will help you right away.";
    }
    ...
    if (lower.includes("spic") || lower.includes("hot") || lower.includes("fire") || lower.includes("dragon")) {
      ...
      return `🌶️ For spice lovers:\n\n${spicy.map(i => `• ${i.name} — ₹${i.price}`).join("\n")}\n\nThe Dragon Chicken Biryani is our spiciest! Warning: not for the faint-hearted 😈`;
    }
    ...
    if (lower.includes("veg") && !lower.includes("non")) {
      ...
      if (filtered.length === 0) return "Hmm, no veg items in that budget. Try increasing a bit? Our cheapest veg option is Veg Pulao at ₹149 🟢";
      return `🟢 Veg options${underBudget ? ` under ₹${underBudget[1]}` : ""}:\n\n${filtered.map(i => `• ${i.name} — ₹${i.price}`).join("\n")}\n\nAll our veg dishes are made with fresh ingredients daily!`;
    }
    ...
    if (lower.includes("non-veg") || lower.includes("nonveg") || lower.includes("non veg") || lower.includes("meat") || lower.includes("chicken") || lower.includes("mutton")) {
      ...
      return `🔴 ${keyword ? keyword.charAt(0).toUpperCase() + keyword.slice(1) : "Non-veg"} options:\n\n${filtered.slice(0, 6).map(i => `• ${i.name} — ₹${i.price}`).join("\n")}${filtered.length > 6 ? `\n\n...and ${filtered.length - 6} more!` : ""}`;
    }
    ...
    if (lower.includes("cheap") || lower.includes("budget") || lower.includes("affordable") || (lower.includes("under") && lower.match(/\d+/))) {
      ...
      if (affordable.length === 0) return `Nothing under ₹${maxPrice} unfortunately. Our most affordable item is Water Bottle at ₹20, and the cheapest meal is Veg Pulao at ₹149 😊`;
      return `💰 Items under ₹${maxPrice}:\n\n${affordable.map(i => `• ${i.name} — ₹${i.price} ${i.type === "veg" ? "🟢" : "🔴"}`).join("\n")}\n\nPro tip: The IT Lunch Combo at ₹199 is unbeatable value! 🎯`;
    }

    // Combos
    if (lower.includes("combo") || lower.includes("deal") || lower.includes("offer") || lower.includes("pack") || lower.includes("sav")) {
      return `🎁 Our combos (massive savings!):\n\n${COMBOS.map(c => `🔥 ${c.name} — ₹${c.price} (Save ₹${c.save}!)\n   ${c.items}`).join("\n\n")}\n\nThe IT Lunch Combo is our most ordered! Which one interests you?`;
    }

    // Price query for specific item
    const priceMatch = lower.match(/(?:price|cost|how much|kitna|rate).*?(?:of |for |is )?(.*)/);
    if (priceMatch || (lower.includes("₹") || lower.includes("price"))) {
      const searchTerm = priceMatch?.[1]?.trim() || lower.replace(/price|cost|how much|kitna|rate|of|for|is|the|₹/gi, "").trim();
      if (searchTerm) {
        const found = allItems.filter(i => i.name.toLowerCase().includes(searchTerm));
        if (found.length > 0) return found.map(i => `${i.name}: ₹${i.price} ${i.type === "veg" ? "🟢" : "🔴"}\n${i.desc}`).join("\n\n");
      }
    }
    ...
    for (const word of words) {
      ...
      if (found.length > 0) return `Here's what I found:\n\n${found.map(i => `• ${i.name} — ₹${i.price} ${i.type === "veg" ? "🟢" : "🔴"}\n  ${i.desc}`).join("\n\n")}\n\nWant to order any of these?`;
    }

    // Fallback
    return "I'm not sure about that, but I can help with:\n\n🍛 Menu & prices\n🌶️ Spice recommendations\n🟢 Veg/non-veg options\n💰 Budget picks & combos\n🍽️ Table booking\n📍 Location & hours\n\nJust ask naturally — like \"best biryani under ₹300\" or \"spicy options\"!";
  };
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-h-[520px] bg-dark-alt border border-white/10 rounded-2xl shadow-2xl shadow-black/40 flex flex-col overflow-hidden animate-fade-in">
          ...
          <div className="border-t border-white/10 p-2.5 flex gap-2">
            ...
            <button onClick={handleSend} className="w-9 h-9 bg-gradient-to-r from-brand to-purple-600 rounded-xl flex items-center justify-center text-white text-sm font-bold hover:opacity-90 transition-opacity">→</button>
          </div>
        </div>
      )}
    </>
