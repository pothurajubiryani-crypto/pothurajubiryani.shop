    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-dark/95 backdrop-blur-xl shadow-lg shadow-black/20 py-3" : "bg-transparent py-5"}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <a href={WHATSAPP_URL} target="_blank" className="btn-wa text-sm !px-5 !py-2.5 flex items-center gap-2">
            <span>📞</span> Order Now
          </a>
        ...
        <div className="flex items-center gap-3 md:hidden">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="w-10 h-10 flex flex-col items-center justify-center gap-1.5">
      ...
      {menuOpen && (
        <div className="md:hidden bg-dark-alt/98 backdrop-blur-xl border-t border-white/5 animate-fade-in">
          <div className="px-6 py-6 flex flex-col gap-4">
            ...
            <a href={WHATSAPP_URL} target="_blank" className="btn-wa text-center mt-2">📞 Order on WhatsApp</a>
          </div>
        </div>
      )}
    </nav>
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-dark">
      <div className="absolute inset-0">
        <Image src="https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=1920&h=1080&fit=crop&q=80" alt="" fill className="object-cover opacity-20" priority />
      ...
      <div className="absolute top-20 right-[10%] w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden opacity-20 animate-float hidden lg:block">
        <Image src="https://images.unsplash.com/photo-1633945274405-b6c8069047b0?w=200&h=200&fit=crop&q=70" alt="" fill className="object-cover" />
      </div>
      <div className="absolute bottom-32 right-[25%] w-20 h-20 rounded-full overflow-hidden opacity-15 animate-float hidden lg:block" style={{ animationDelay: "1s" }}>
        <Image src="https://images.unsplash.com/photo-1610057099431-d73a1c9d2f2f?w=200&h=200&fit=crop&q=70" alt="" fill className="object-cover" />
      </div>
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-32 grid lg:grid-cols-2 gap-12 items-center">
        <div className="animate-fade-up">
          ...
          <div className="flex flex-wrap gap-4 mb-12">
            <a href={`${WHATSAPP_URL}?text=${encodeURIComponent("Hi! I'd like to order.")}`} target="_blank" className="btn-wa text-base flex items-center gap-2 !px-8 !py-4">
              💬 Order on WhatsApp
            </a>
...
/* ─── MARQUEE ─── */
function Marquee() {
  const text = "🔥 5 Lakh Customers in 7 Months!  |  Chicken Biryani + Raita + Drink @ just ₹199  |  Order Direct & Save 10%!  |  ";
  return (
...
function MenuCard({ item }: { item: MenuItem }) {
  ...
  const handleAdd = () => { cart.add(item.id, item.name, item.price); setAdded(true); setTimeout(() => setAdded(false), 600); };
  return (
    <div className="group bg-dark-alt border border-white/5 rounded-2xl overflow-hidden hover:border-gold/20 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/20">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleAdd} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all duration-300 ${added ? "bg-whatsapp text-white scale-110" : "bg-brand/20 text-brand-light border border-brand/30 hover:bg-brand hover:text-white"}`}>{added ? "✓" : "ADD"}</button>
        </div>
      </div>
    </div>
  );
...
function SmartRecs() {
  ...
  const recs = hasBiryani && !hasSides ? MENU.sides.slice(0, 3) : !hasBiryani ? MENU.biryani.filter(i => i.badge === "best").slice(0, 3) : MENU.chinese.filter(i => i.badge === "best" || i.badge === "hot").slice(0, 3);
  if (recs.length === 0) return null;
  return (
    <div className="mb-10 bg-gradient-to-r from-gold/5 to-brand/5 border border-gold/10 rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4"><span className="text-lg">🤖</span><span className="text-sm font-semibold text-gold">People also ordered</span></div>
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
        {recs.map(item => (
          <div key={item.id} className="flex-shrink-0 flex items-center gap-3 bg-dark/50 border border-white/5 rounded-xl p-3 min-w-[220px]">
            <div className="relative w-14 h-14 rounded-lg overflow-hidden flex-shrink-0"><Image src={item.img} alt={item.name} fill className="object-cover" /></div>
            <div className="flex-1 min-w-0"><div className="text-xs font-medium text-white truncate">{item.name}</div><div className="text-sm font-display text-gold">₹{item.price}</div></div>
    ...
    <section id="menu" className="section-pad bg-dark">
      <div ref={ref} className="reveal max-w-7xl mx-auto px-4">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 mb-8 scrollbar-hide">
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActive(cat.id)} className={`flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 ${active === cat.id ? "bg-brand text-white shadow-lg shadow-brand/30" : "bg-white/5 text-white/60 border border-white/10 hover:border-white/20"}`}>
              <span>{cat.icon}</span>{cat.label}<span className={`text-xs ${active === cat.id ? "text-white/70" : "text-white/30"}`}>({cat.count})</span>
            </button>
          ))}
        </div>
        <SmartRecs />
          ...
          {COMBOS.map((combo, i) => (
            <div key={combo.id} className={`reveal reveal-delay-${i + 1} group relative bg-dark border border-white/5 rounded-2xl overflow-hidden hover:border-gold/20 transition-all duration-500 hover:-translate-y-2`}>
              ...
              <div className="p-5">
                ...
                <div className="flex items-end gap-3 mb-3"><span className="font-display text-2xl text-white">₹{combo.price}</span><span className="text-sm text-white/30 line-through">₹{combo.was}</span></div>
                <div className="flex items-center gap-2 mb-4"><span className="text-xs font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full">SAVE ₹{combo.save}</span></div>
                <div className="flex gap-2">
                  <a href={`${WHATSAPP_URL}?text=${encodeURIComponent(`Hi! I want the ${combo.name} @ ₹${combo.price}`)}`} target="_blank" className="flex-1 btn-wa !py-2 text-center text-sm">💬 Order</a>
...
function OrderBanner() {
  ...
  return (
    <section className="py-16 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10"><Image src="https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=1920&h=400&fit=crop&q=60" alt="" fill className="object-cover" /></div>
      <div ref={ref} className="reveal relative z-10 max-w-4xl mx-auto px-4 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-8 mb-8">
          {["No App Commission", "10% Cheaper", "Faster Delivery", "Direct Support"].map(t => (
            <div key={t} className="flex items-center gap-2 text-sm text-white/80"><span className="text-whatsapp">✓</span> {t}</div>
          ))}
        </div>
        <a href={`${WHATSAPP_URL}?text=${encodeURIComponent("Hi! I want to place a direct order.")}`} target="_blank" className="btn-wa !px-10 !py-4 text-lg inline-flex items-center gap-2">💬 Order on WhatsApp Now</a>
      </div>
    </section>
  );
}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {REVIEWS.map((r, i) => (
            <div key={i} className={`reveal reveal-delay-${(i % 4) + 1} bg-dark-alt border border-white/5 rounded-2xl p-6 hover:border-gold/20 transition-all duration-500`}>
              <div className="flex gap-0.5 mb-4">{[...Array(r.rating)].map((_, j) => <span key={j} className="text-gold text-sm">★</span>)}</div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div><div className="text-sm font-semibold text-white">{r.name}</div><div className="text-xs text-white/30">{r.source} Review</div></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div ref={ref} className="reveal max-w-7xl mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="rounded-2xl overflow-hidden border border-white/10 h-80 lg:h-auto">
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3806.2!2d78.3528!3d17.4611!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMTfCsDI3JzQwLjAiTiA3OMKwMjEnMTAuMSJF!5e0!3m2!1sen!2sin!4v1" width="100%" height="100%" style={{ border: 0, minHeight: 320 }} allowFullScreen loading="lazy" />
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2.5">
              <a href={WHATSAPP_URL} target="_blank" className="text-sm text-white/40 hover:text-whatsapp transition-colors">💬 WhatsApp</a>
        ...
        <div className="border-t border-white/5 pt-6 text-center text-xs text-white/20">© 2025 Pothuraju Biryani. All rights reserved. | Kondapur, Hyderabad | Built with ❤️</div>
      </div>
    </footer>
  );
}
        ...
        <div className="flex flex-col h-full">
          ...
          {entries.length > 0 && (
            <div className="border-t border-white/10 p-5 space-y-3">
              <div className="flex justify-between items-center mb-2"><span className="text-white/50">Total</span><span className="font-display text-2xl text-white">₹{cart.total}</span></div>
              <button onClick={waCheckout} className="w-full btn-wa !py-3.5 text-center flex items-center justify-center gap-2">💬 Checkout on WhatsApp</button>
              <button className="w-full btn-primary !py-3.5 text-center flex items-center justify-center gap-2 opacity-60 cursor-not-allowed" title="Coming soon">💳 Pay Online (Coming Soon)</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3 items-end">
        <a href={`${WHATSAPP_URL}?text=${encodeURIComponent("Hi! I'd like to book a table.")}`} target="_blank" className="flex items-center gap-2 bg-whatsapp text-white pl-4 pr-5 py-3 rounded-full shadow-lg shadow-whatsapp/30 hover:scale-105 transition-transform">
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[340px] max-h-[480px] bg-dark-alt border border-white/10 rounded-2xl shadow-2xl shadow-black/40 flex flex-col overflow-hidden animate-fade-in">
          <div className="bg-gradient-to-r from-brand to-brand-dark px-4 py-3 flex items-center gap-3">
            ...
            <div><div className="text-sm font-bold text-white">Pothuraju Bot</div><div className="text-[10px] text-white/60 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-whatsapp inline-block" /> Online</div></div>
          </div>
          <div ref={chatRef} className="flex-1 overflow-y-auto p-3 space-y-2.5 min-h-[260px] max-h-[320px]">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] px-3 py-2 rounded-xl text-sm whitespace-pre-line ${m.from === "user" ? "bg-brand text-white rounded-br-sm" : "bg-white/5 text-white/80 border border-white/5 rounded-bl-sm"}`}>{m.text}</div>
              </div>
            ))}
          </div>
          <div className="border-t border-white/10 p-2.5 flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} placeholder={step === 0 ? "Type 1, 2, 3, or 4..." : "Type your answer..."} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-gold/40" />
